...
use advent_of_code_2024_day_12::{
    coordinate::{Board, Coordinate, MoveDirection},
    reading::read,
};
...
fn calculate(coordinate: Coordinate, visited: &mut HashSet<Coordinate>) -> AreaAndPerimeter {
    ...
    dfs(coordinate, visited, &mut area_perimeter);
    area_perimeter
}
...
fn dfs(
    ...
    if !visited.contains(&coordinate) {
        ...
        area_perimeter.perimeter += coordinate.perimeter();
        for direction in [
            MoveDirection::Up,
            MoveDirection::Down,
            MoveDirection::Left,
            ...
            if let Some(new_coordinate) = coordinate.move_to(&direction) {
                if new_coordinate.get_value() == coordinate.get_value() {
                    dfs(new_coordinate, visited, area_perimeter);
                }
            }
        }
    }
